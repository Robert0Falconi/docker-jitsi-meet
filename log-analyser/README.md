# Rob Meet LOG ANALYSER - Grafana Loki and OpenTelemetry Integration
...
This project demonstrates how to configure and use Grafana Loki with OpenTelemetry to collect, parse, and visualize log data from Rob Meet components. It includes:
...
2. **Update Rob Meet Docker Compose Configuration:**

To enable log collection and analysis, you need to modify the `docker-compose.yml` file for Rob Meet components. Add the following configuration to each Jitsi service within the `docker-compose.yml` file:
...
3. **Start the Docker containers:**
   ...
   To start all necessary services, including Rob Meet components, Grafana, Loki, and OpenTelemetry, run:

    ```bash
    docker-compose -f docker-compose.yml -f log-analyser.yml -f grafana.yml up -d
    ```

   - This command will start the Rob Meet components from `docker-compose.yml`, the log analysis tools from `log-analyser.yml`, and Grafana from `grafana.yml`. The logs from Rob Meet components will automatically be sent to Grafana through Loki.
...
3. **Dashboard Setup:**
   ...
   The dashboards for Rob Meet components are pre-configured and will be automatically available in Grafana. You can explore these dashboards to view and analyze logs.

## Usage

- **Log Parsing and Visualization:** After setting up, use Grafana to explore and visualize your logs. Check the pre-configured dashboards and panels to monitor and analyze log data from Rob Meet components effectively.
